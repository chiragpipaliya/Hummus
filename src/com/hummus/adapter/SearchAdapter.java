...
import com.hummus.R;
import com.hummus.SearchdetailsActivity;
import com.hummus.model.ListModel;
import com.hummus.share.Share;
import com.imageloader.ImageLoader;

public class SearchAdapter extends BaseAdapter implements Comparator<ListModel>,Filterable {
 ...
 public SearchAdapter(Activity activity,ArrayList<ListModel>nameList){
...
private class ValueFilter extends Filter {
	...
	protected FilterResults performFiltering(CharSequence constraint) {
		...
		if (constraint != null && constraint.length() > 0) {
			ArrayList<ListModel> filterList = new ArrayList<ListModel>();
			for (int i = 0; i < mStringFilterList.size(); i++) {
				if ((mStringFilterList.get(i).getName().toUpperCase()).contains(constraint.toString().toUpperCase())) {

					ListModel country = new ListModel(mStringFilterList
							.get(i).getEvent_id(), mStringFilterList.get(i)
							.getName(), mStringFilterList.get(i)
							.getAddress(), mStringFilterList.get(i)
							.getMiles(),mStringFilterList.get(i).getLatitude(),mStringFilterList.get(i).getLongitude(),mStringFilterList.get(i).getRating(),
							mStringFilterList.get(i).getTelephone(),mStringFilterList.get(i).getWebsite(),
							mStringFilterList.get(i).getDes());

					filterList.add(country);
				}
				else if(mStringFilterList.get(i).getDes().toUpperCase().contains(constraint.toString().toUpperCase())){
					ListModel country = new ListModel(mStringFilterList
							.get(i).getEvent_id(), mStringFilterList.get(i)
							.getName(), mStringFilterList.get(i)
							.getAddress(), mStringFilterList.get(i)
	...
	protected void publishResults(CharSequence constraint,
		...
		namelist = (ArrayList<ListModel>) results.values;
